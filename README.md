...
> **Tip for Development:** Since the local environment usually doesn't have an email server configured, the activation link sent during registration is logged to the `kyc` service logs. You can find it by running:
>
> ```bash
> docker compose logs kyc | grep "Activation link"
> ```
