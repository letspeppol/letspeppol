...
class CompanySearchTests {
    ...
    void searchCompanies() {
        ...
        // Search by name
        List<CompanySearchResponse> results = companyService.search(identifier, null, null, "Acme");
        assertThat(results).hasSize(2);
        assertThat(results).extracting(CompanySearchResponse::name).contains("Acme Corp", "Acme Limited");

        // Search by VAT
        results = companyService.search(identifier, "BE0987654321", null, null);
        assertThat(results).hasSize(1);
        assertThat(results.get(0).name()).isEqualTo("Beta Inc");

        // Search by Peppol ID
        results = companyService.search(identifier, null, "0208:1122334455", null);
        assertThat(results).hasSize(1);
