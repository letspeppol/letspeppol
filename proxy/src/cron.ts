...
export async function checkForIncomingDocs(): Promise<void> {
  ...
  await Promise.all(
    docIds.map(async (docId: string) => {
      ...
      if (!res.ok) {
        console.error(
          'Failed to fetch incoming document:',
          docId,
          res.statusText,
        );
        return;
      ...
      Object.keys(expect).forEach((key) => {
        if (docDetails[key] !== expect[key]) {
          console.error(
            `Document ${docId} does not match expected ${key}:`,
            docDetails[key],
          );
        }
      });
      ...
      const confirmRes = await fetch(
        `https://apitest.scrada.be/v1/company/${process.env.SCRADA_COMPANY_ID}/peppol/inbound/document/${docId}/confirm`,
        {
          method: 'PUT',
          headers: {
            'X-Api-Key': process.env.SCRADA_API_KEY!,
      ...
      if (!confirmRes.ok) {
        console.error(
          'Failed to confirm incoming document:',
          docId,
          confirmRes.statusText,
        );
        return;
      }
